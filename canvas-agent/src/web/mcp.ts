...
const WEB_MCP_INSTRUCTIONS = [
  ...
  'After creating or opening a project, wait for its complete snapshot and read state again. The connected page automatically enables bounded non-billing writes when the Runtime lease is available.',
  'Read state once before a change and reuse its projectId and revision.',
  'The project remains read-only when another editor owns its Runtime lease or the automatic handoff fails.',
