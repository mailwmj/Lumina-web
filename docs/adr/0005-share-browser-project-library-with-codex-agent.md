...
一台电脑上的 Lumina 只有一个 Web 项目库：用户手动打开的、当前安装稳定的 Lumina URL 与 Codex Agent 入口必须在用户已连接的同一 Chrome 上下文中访问同一份 IndexedDB 项目、历史和资产。Codex Lumina 插件当前在该连接 Chrome 中打开或聚焦返回的地址；没有连接时请求连接并停止，不得创建未声明的第二个浏览器项目库。companion 只作为受控桥接边界。这样保留浏览器作为项目和凭据事实源，同时避免两个入口产生不可见的分叉项目库。
