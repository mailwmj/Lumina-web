...
一台电脑上的 Lumina 只有一个 Web 项目库：用户手动打开的、当前安装稳定的 Lumina URL 与 Codex Agent 入口必须在用户已连接的同一浏览器上下文中访问同一份 IndexedDB 项目、历史和资产。普通 Agent 使用不再创建会话级 loopback Web 应用或静默退回独立 in-app browser；companion 只作为受控桥接边界。这样保留浏览器作为项目和凭据事实源，同时避免两个入口产生不可见的分叉项目库。
