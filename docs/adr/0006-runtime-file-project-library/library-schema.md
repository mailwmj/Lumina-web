...
[`admission-registry-v1.json`](./admission-registry-v1.json) is the sole versioned, machine-readable authority for v1 admitted source records, project/history documents, node variants, edge data, `AssetMetadata`, `sourceMetadata`, provider parameters, MIME grammar, unknown-member behavior, normalization, secret handling, and numeric limits. An implementation must load that exact format/version, reject a duplicate/unknown field or node type, and reject a registry version it does not implement. The registry is intentionally closed except for the registry-defined `AssetSourceMetadata` scalar-map rule below: TypeScript index signatures and the current source adapters do not authorize an unlisted persisted field. Any later field, MIME, node type, size, or sanitization rule requires a new registry version and a new admission path; it cannot be accepted as an untyped compatibility pass-through.

`AssetSourceMetadata` is the one compatibility-preserving scalar-map exception, not a nested arbitrary-object exception. A parsed map may contain at most `limits.maxSourceMetadataEntries` (32) entries and its complete canonical `AssetMetadataDocumentV1` must remain within `limits.maxAssetMetadataDocumentBytes` (65,536 bytes). Each key must match exactly `^[A-Za-z][A-Za-z0-9_.-]{0,63}$`; its original ASCII spelling and case are preserved, while the admission sensitive-name classifier separately lowercases and retains only ASCII letters/digits to decide whether that key is forbidden. A sensitive key is rejected, never omitted or renamed. This admits the current scalar-map values such as `fileName`, `sourceMimeType`, provider annotations, and the current `storyboardMetadata` JSON **string** without treating that string as a nested `StoryboardMetadata` object.

Each map value is exactly one string, finite number, boolean, or `null`. A string is at most `limits.maxSourceMetadataStringBytes` (32,768 UTF-8 bytes), is preserved byte-for-byte after the required secret/URL/token scan, and rejects rather than redacts an unsafe value. A finite number uses RFC 8785/JCS numeric normalization (`-0` becomes `0`); booleans and `null` are preserved exactly. Arrays, objects, duplicate keys, non-finite numbers, unknown value types, and keys outside that grammar are `project_secret_admission_failed`. Canonical serialization sorts only for the canonical bytes; it does not rename safe map keys or discard safe scalar entries. Thus a valid current scalar-map entry round-trips through the admitted projection, while an arbitrary nested value or credential-bearing key/value cannot enter a file-library metadata document, digest, staging record, or migration evidence.

`optionalDerivedDisplayUrl` is a different compatibility rule for node/frame display fields (`imageUrl`, `videoUrl`, `audioUrl`, `previewImageUrl`, `previewVideoUrl`, and `lastFrameImageUrl`). They are optional persisted facts, never asset identities. When the matching stable `assetId` exists, current project persistence omits its main display URL; when the main or `previewAssetId` exists it omits the preview URL; and when `lastFrameAssetId` exists it omits the last-frame URL. Persisted history omits every one of these fields regardless of backing state. Their absence is therefore valid for current project, history, and archive fixtures. If an unbacked live-project display URL is present, it remains an admitted URL field and must satisfy the registry's safe `http:`/`https:` rule; `data:` and `blob:` are not durable facts. A display URL does not create reachability, ownership, or a substitute for an asset Blob, and the target must not reconstruct or retain a derived URL merely because an earlier in-memory node had one.
