...
type RuntimeCommandSubject = Readonly<{
...
type RuntimeCommandContext = Readonly<{
  /** Runtime-issued rc_<libraryRootId>_<base-10 sequence>; retries use the same commandId. */
  commandId: string;
  ...
  authorization: {
    ...
    /** RFC8785-JCS-SHA256-v1 of the admitted action-specific request body. */
...
type RuntimeCommandReplayFailure =
  | { code: 'command_body_mismatch'; retryable: false }
  | { code: 'command_outcome_expired'; retryable: false }
  | { code: 'command_recovery_failed'; retryable: false };

authorizeRuntimeCommand({ action, subject, expectedCatalog, commandRequestSha256 }):
  Promise<
    | { code: 'authorized'; context: RuntimeCommandContext }
    | { code: 'stale_catalog'; actualCatalog: CatalogRevision }
    | { code: 'authorization_denied' }
    | { code: 'command_capacity_exhausted'; retryable: false }
    | { code: 'command_id_exhausted'; retryable: false }
  >;
...
type RuntimeLibraryTransactionReconciliation = {
  ...
  expectedCatalog: CatalogRevision;
  capability: {
    format: 'lumina-library-import-reconciliation-authorization';
    version: 1;
    allocationId: string;
    operationId: ImportOperationId;
...
type RuntimeLibraryTransaction = {
  ...
  /** The caller allocation and operation are paired by the durable import ledger. */
  allocationId: string;
...
`CatalogRevision` is the exact value pinned from one validated `head.json` and its named catalog: all three fields must equal under the write lease. Every mutating public target call above carries `RuntimeCommandContext`; `authorizeRuntimeCommand` is the only issuer of its command ID and proof. It first validates the bridge authorization, action, exact three-member subject, expected catalog, and a candidate `commandRequestSha256`; it then allocates `rc_<libraryRootId>_<base-10 sequence>` under the library write lease, durably records it, and returns the context. A caller cannot choose, recycle, or retarget a command ID. `saveSnapshot`, `updateViewport`, and `rename` require `project-mutation`; `delete` requires `project-delete`; `restoreProject` requires `project-restore`; `writeRuntimeAsset` requires `asset-write`; candidate replacement/request/restore requires `asset-lifecycle`; `restoreAssetFromTrash` requires `asset-trash-restore`; archive allocation/publication require `library-import`; and `emptyTrash` requires `empty-trash`. Allocation and publication are distinct commands with distinct runtime-issued command IDs, even though both are catalog-pinned `library-import` actions. A duplicated `expectedCatalog` in an import or lifecycle precondition must byte-for-byte equal `context.expectedCatalog`.

`commandRequestSha256` is `RFC8785-JCS-SHA256-v1` over the exact non-self-referential `RuntimeCommandRequestV1` value `{ format: 'lumina-runtime-command-request', version: 1, action, expectedCatalog, subject: { projectId, assetId, deletionId }, body }`. All three `subject` members are present and use `null` when irrelevant. `commandId` is intentionally outside this preimage because the runtime issues it only after this body digest has been presented; the durable `(commandId, commandRequestSha256)` pair and authenticated proof bind it to one body. `body` is action-specific and has no implicit defaults: a project mutation carries its kind, project ID, expected revision, and either the complete parsed/admitted project-plus-history value, parsed admitted viewport, admitted name/timestamp, no extra value for delete, or the exact deletion ID/manifest digest for restore; an asset write carries its project/revision, admitted metadata input, and `{ byteCount, rawBytesSha256 }` rather than Blob bytes; an asset lifecycle mutation carries its complete sorted precondition and normalized mutation; empty-trash carries its deletion ID/manifest digest; an import allocation carries its allocation ID; and an import publication carries that allocation ID, operation ID, expected catalog, `admittedImportSha256`, and import-specific `requestSha256`. Asset-ID candidate arrays and `expectedAssets` are UTF-8-byte sorted with duplicates rejected before this preimage. The full parsed/admitted record, not host JSON string spelling or object-key order, is used for project/history/metadata values. The existing import `requestSha256` remains its operation-payload identity; `commandRequestSha256` binds the authenticated command that asks to allocate or publish it.

The bridge must apply the same registry admission before asking for authorization, and the runtime independently repeats admission before it records a command ledger entry, hashes Blob bytes, stages, or publishes. An unsafe input returns its established admission error with no ledger entry, request digest, candidate, or raw-derived evidence. For an otherwise admissible submitted body, the runtime recomputes the exact request value and requires it to equal both the authorization's `commandRequestSha256` and its own recomputation. A different safe body, action, subject, expected catalog, source-to-target selector, Blob byte digest/count, or lifecycle set under the same command ID returns non-retryable `command_body_mismatch` before staging. The runtime never stores a raw request, authorization proof, Blob bytes, or a secret-derived request digest in this ledger.

`control/runtime-command-ledger.json` is the canonical non-secret `RuntimeCommandLedgerV1`: `{ format: 'lumina-runtime-command-ledger', version: 1, namespace: <libraryRootId>, lastAllocatedSequence: <safe integer>, entries: <commandId-sorted entries> }`. Each entry contains only `commandId`, action, exact subject, expected catalog, `commandRequestSha256`, authorization expiry, `state: 'authorized' | 'pending' | 'completed' | 'recovery_failed'`, one opaque transaction ID when staged, intended catalog when applicable, a canonical safe response/result when terminal, and its retention timestamps. It is capped at 4,096 live entries; eligible exact expiry cleanup occurs before issuing another ID, otherwise authorization returns non-retryable `command_capacity_exhausted`, and a safe-integer sequence exhaustion returns `command_id_exhausted`. The high water is never reduced, so a command ID at or below it that has no retained entry is always non-retryable `command_outcome_expired`, never a new authorization. A never-submitted `authorized` entry expires with its proof; a `completed` or `recovery_failed` entry replays its exact canonical result through `completedAt + 24 * 60 * 60 * 1000` and is then removed by an exact durable ledger update. A matching retained completed request returns that original result, including its original catalog/revision; it never reruns a mutation against a later head.

Crash recovery is keyed only by that one ledger entry and its named transaction. Before the `authorized` ledger flush, no ID is issued. After it but before staging, the same valid body/context advances the same entry; an expired authorization cannot be renewed and requires a new command. `pending` with no durable `publish.json` resumes preparation only for that same digest; `pending` with a valid named publish record but no named head resumes only that publication; and a head naming the intended catalog completes and replays the stored result before the matching staging control record is discarded. A missing, mismatched, or damaged named candidate is quarantined only through its own record, changes that entry to terminal `recovery_failed`, and returns `command_recovery_failed`; it never scans or retries another command. Thus a crash cannot let a changed body inherit an old command ID, and compaction cannot make that ID reusable.

A stale catalog returns `stale_catalog` with the one current complete revision, a stale project/asset precondition returns its typed stale result, and an expired/missing/wrong-action/wrong-subject proof returns `authorization_denied` before staging. Browser adapters attempting a frozen IndexedDB store instead return the separate non-retryable `FrozenBrowserStoreWrite`; runtime file commands never pretend that result was a catalog conflict. A caller identified as an MCP App widget receives `WidgetWriteDenied` before authorization or staging until #48 supplies and independently accepts a widget write contract. That denial is a boundary, not a #45 release prerequisite.

`applyLibraryTransaction` is the sole target bulk command. The runtime, not an arbitrary client UUID, allocates every `ImportOperationId` from the permanent `control/import-operation-ledger.json` under the library write lease. Its exact canonical `LibraryImportOperationLedgerV1` value is `{ format: 'lumina-library-import-operation-ledger', version: 1, namespace: <library.json importOperationNamespace>, lastAllocatedSequence: <safe integer>, allocations: <allocationId-sorted active allocation records> }`; each active record is `{ allocationId, operationId, expectedCatalog, requestSha256: <null before claim or exact claimed request digest>, allocatedAt, expiresAt, state: 'allocated' | 'claimed' }`. An ID is exactly `li_<namespace>_<base-10 sequence>` with no leading zero. A caller first persists one lowercase UUID v4 `allocationId` beside its pending archive and calls `allocateLibraryImportOperation` with an import-authorized context. A matching still-live allocation returns the original ID; a different catalog/context is `operation_mismatch`. For a new allocation the runtime increments `lastAllocatedSequence`, writes and flushes the complete ledger before returning the ID, and only then accepts the first archive frame. A crash before that flush allocates nothing; a crash after it may leave a permanently consumed gap, which is intentional. An unclaimed allocation expires exactly 24 hours after `allocatedAt` and is removed only by an exact maintenance update; the high-water value never decreases. A claimed allocation remains until its terminal receipt is eligible for expiry. The active allocation list is capped at 128; capacity exhaustion rejects before a sequence increment.

On submission and reconciliation, the runtime parses the namespace/sequence, requires its namespace to equal `library.json`, and requires `1 <= sequence <= lastAllocatedSequence`. A sequence above the high water or malformed namespace is `operation_mismatch`; a consumed sequence with no live allocation or retained receipt is the non-retryable `operation_outcome_expired`, never a fresh import. Thus receipt compaction does not make an old logical import ID reusable. A lost allocation response is replayed by the same `allocationId` until its exact allocation expiry; after that the caller must explicitly authorize a new allocation and gets a higher ID. A lost publication response is replayed only by the authenticated reconciliation capability below while its receipt exists. That logical ID is never a filesystem path. The runtime must run `lumina-project-migration-admission-v1` over every imported project/history/AssetMetadata value before it creates a receipt, staging entry, canonical digest, or target file. It then computes `admittedImportSha256` over the complete RFC 8785 value `{ format: 'lumina-admitted-project-import', version: 1, admission: 'lumina-project-migration-admission-v1', archive: { format: 'lumina-project-export', version: <admitted archive version> }, projects: <UTF-8-ID-sorted { sourceProjectId, revision, projectSha256, historySha256 }[]>, assets: <UTF-8-ID-sorted { sourceAssetId, sourceProjectId, metadataSha256, bytesSha256, byteCount, lifecycleState }[]> }`, where the project/history/metadata digests are the admitted canonical values and only `bytesSha256` is `raw-bytes-sha256`. `requestSha256` is the digest of `{ format: 'lumina-library-import-request-v1', version: 1, operationId, expectedCatalog, admittedImportSha256 }`. No raw archive/text/metadata hash or raw archive-entry digest is stored in the receipt, staging, report, or request preimage. Reusing an `operationId` with a different admitted payload or expected catalog returns `operation_mismatch` before publication.
...
### Deterministic target ID allocation and reference remapping

This section is the sole target authority for source-to-target IDs in a library import, browser import, project trash restore, and asset trash restore. The source set is the complete admitted input, not archive entry order or an object iteration order. Project IDs and asset IDs have distinct namespaces: a project and an asset may have the same source string, but duplicate source project IDs or duplicate source asset IDs in one operation are `id_conflict` before staging, hashing a target mapping, or a visible change. A history record has no independently allocated ID: its source owner `StoredHistoryRecord.projectId` must equal the source project ID, and its target owner/key is the paired target project ID. If a history/project payload contains a registry-declared `projectId` reference, it is rewritten through that same project map.

Under the one library write lease, the runtime first revalidates the pinned catalog, reads its complete occupied project and asset ID sets, and processes each distinct source namespace in UTF-8 byte order. For each source ID it first tries the unmodified source string. If that value is already occupied by the pinned target catalog or a target value allocated earlier in this operation, it tries exactly `${sourceId}~import-1`, then `${sourceId}~import-2`, and so on using the smallest positive safe-integer suffix that is unoccupied. It never strips or parses a pre-existing suffix: a colliding source `alpha~import-1` next tries `alpha~import-1~import-1`. Every candidate must remain a valid `requiredId` and at most 256 UTF-8 bytes; the allocator never truncates. A required suffix above `Number.MAX_SAFE_INTEGER`, an invalid/overlong candidate, a duplicate source, or a failed resolved-target absence compare-and-swap is `id_conflict` with no target publication. The held lease prevents ordinary concurrent allocation, while the final compare-and-swap makes a corrupted/reentrant implementation fail closed rather than overwrite an occupied ID.

For a restore, the API's `projectId`/authorization subject identifies the source/original project selected by the trash manifest. It must not test `expectedRevision: 'absent'` against that original value before allocation: a live project at that value is precisely the collision case. After it validates the manifest and snapshots the target catalog under the lease, it applies the algorithm to the source project and every restored asset, then tests `expectedRevision: 'absent'` against the resolved target project ID and resolved target asset IDs. The same rule applies when an asset-trash restore collides. A retry/restart never recomputes a suffix after another publication: it rereads the exact stored mapping and revalidates only the resolved targets. A collision discovered after a stored mapping exists is an `id_conflict`/typed stale result, not permission to pick a different suffix.

Before any target head can publish, `staging/<transactionId>/publish.json` stores the complete UTF-8-source-ID-sorted project, history-owner, and asset maps. A library-import receipt and `BrowserImportOperationV1.published` store the identical maps. A restore additionally materializes immutable `trash/<deletionId>/restores/<transactionId>.json` before its head replacement with `{ format: 'lumina-library-restore-mapping', version: 1, deletionId, sourceProjectId, targetProjectId, projectMappings, historyOwnerMappings, assetMappings, publishSha256 }`; it is a trash/recovery root until the paired restore and trash-retention cleanup are terminal. The maps are canonical, complete, and exact; a lost reply or crash returns/reuses this material, never a recomputed map. The compatibility helper currently named `resolveImportedIds` demonstrates the existing `~import-N` spelling, but its archive-order allocation and limited reference traversal are current browser behavior, not target conformance evidence.

The runtime rewrites the complete admitted project document and every retained history snapshot before their target canonical hashes. It changes the project record ID, history owner/key, every `AssetMetadata.assetId`/`projectId`, every registry `assetReference` (`assetId`, `previewAssetId`, `lastFrameAssetId`), and every registry `assetReferenceArray` (`referenceImageIds`, `referenceAudioIds`, `referenceVideoIds`), as well as every registry-declared `projectId` reference. It rejects an unknown/unclassified identifier-shaped member rather than performing a recursive name guess. Post-remap admission/reference closure must prove that every rewritten target reference resolves to exactly one mapped or pre-existing target fact. Conformance fixtures must fix source ordering, in-batch duplicates, separately scoped project/asset collisions, the 256-byte and safe-integer failure branches, imported and restored histories, all scalar and array reference locations, a crash before/after mapping persistence, and this example: existing project IDs `alpha`, `alpha~import-1`; source project IDs `alpha`, `alpha~import-1`, `zeta`; resulting project map `alpha -> alpha~import-2`, `alpha~import-1 -> alpha~import-1~import-1`, `zeta -> zeta`.

After the runtime has allocated the new `commitId` and sequence, but before it hashes the complete catalog, it creates the exact `publishedCatalogContentSha256` domain defined above from that catalog's commit identity, attachment, project map, and asset map. It then creates the exact canonical inner `lumina-library-completed-import-receipt` shown above and computes `receiptSha256` over that inner value only. It puts `{ receipt, receiptSha256 }` into the catalog and computes the catalog's `commitSha256` once. The receipt deliberately has no `publishedCommitSha256` or `catalogDigest`, so it cannot require a SHA-256 fixed point. Every later full catalog copies each unexpired envelope byte-for-byte. The inner receipt is keyed by `operationId` and contains the originating `allocationId`, request hash, preallocated published commit ID/sequence, receipt-excluded catalog-content digest, retention time, and complete project and asset mappings. It is retained for 30 days from `publishedAt` and may be removed only by a later serialized catalog publication after `retainedUntil`. Staging cleanup must never delete the only mapping. This is bounded reconciliation evidence rather than an unbounded operation log.

If a response is lost after step 5, the client first obtains a fresh short-lived `lumina-library-import-reconciliation-authorization` on an authenticated bridge session, then calls `reconcileLibraryTransaction` with that capability. The capability is issued only after the runtime validates the originating `allocationId`, `operationId`, exact `requestSha256`, and original `expectedCatalog` against the active allocation or retained receipt; issuance itself returns no mappings, receipt state, or publication result. Its signed transport proof is bound to those five claims plus `bridgeSessionId`, `issuedAt`, and `expiresAt`, is valid for at most five minutes and never beyond the matching allocation/receipt retention, and is never persisted or logged. The query duplicates `allocationId`, `operationId`, and `expectedCatalog`; all three must byte-for-byte equal the capability. An invalid/expired proof, wrong session, or missing import/reconcile authorization returns `authorization_denied` before the runtime reads a receipt or exposes `not_published`; a different allocation, operation ID, request digest, or catalog claim returns `operation_mismatch` with no mappings. Thus an operation ID plus catalog alone cannot disclose a project's or asset's target IDs. A reconnect after its capability expires must obtain a new user-authorized session-bound capability for the same tuple; it never falls back to a blind retry or a persistent bearer token.

Only after that authorization check does the runtime reread and validate `head.json` and its catalog. An unexpired matching envelope first verifies `receiptSha256`, its `allocationId` and `requestSha256`, then opens only `commits/<receipt.publishedCommitId>.json`, recomputes that catalog's SHA-256 and receipt-excluded `publishedCatalogContentSha256`, requires its `commitId`/sequence to equal the receipt, and requires it to contain the exact same envelope. It then replays `AppliedLibraryImport` with the computed original `CatalogRevision` and every mapping, even if later catalog commits have advanced the head. If the receipt is absent while the validated head still equals the original expected catalog and the exact allocation remains claimed/live with the same request digest, `not_published` proves that the import was not visible and the caller may retry the same operation. If a syntactically valid operation sequence is at or below the durable high water but neither a live allocation nor a retained receipt exists, an otherwise authorized reconciliation returns `operation_outcome_expired`; clients and MCP must never replay that operation automatically. `unknown_outcome` is reserved for a damaged/contradictory ledger or catalog that must enter read-only maintenance, not ordinary receipt expiry. A new import then requires a new explicit authorization, allocation ID, operation ID, and pinned catalog. This protocol, rather than staging-directory inspection, prevents a lost reply from creating a double import or disclosing mappings to an unauthenticated caller.
