...
| Codex plugin | Desktop installation, explicit Codex import, Node.js >=18, connected Chrome | `canvas_open` through the plugin MCP | Same installed Runtime and managed library | User's connected Chrome | Codex manages the MCP process; user does not run source commands. |
