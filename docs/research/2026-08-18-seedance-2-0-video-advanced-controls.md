...
| --- | --- | --- | --- |
| Output audio | `generate_audio: boolean` (default `true`) | `hasAudio` becomes `extraParams.hasaudio`, then `webVideoApi.ts` serializes it as `generate_audio`. | Direct API control. `true` asks the model to generate synchronized voice, sound effects, and background music; `false` requests silent output. This is separate from an audio reference input (`content` item with role `reference_audio`).[^create][^tutorial] |
