...
Use `canvas_list_projects` when no project is open. `canvas_create_project` and `canvas_open_project` each require explicit approval in that Lumina page. After either action succeeds, wait for the page to publish the complete new project snapshot and call `canvas_get_state`. The connected page automatically enables bounded non-billing writes when the Runtime lease is available; never reuse the previous project's ID, revision, write grant, or run approval.
...
The project is writable by default for bounded non-billing operations when the bridge and Runtime lease are ready. It remains read-only when another editor owns the lease or the automatic handoff fails. Submit one `canvas_propose_changes` change set at a time. Only allowed create, update, move, and connection operations are available; project deletion, credential reads, arbitrary result-node creation, and arbitrary file reads are unavailable.
