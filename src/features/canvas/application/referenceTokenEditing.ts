...
const REFERENCE_PATTERNS: PatternMatch[] = [
  { prefix: '@图', type: 'image', prefixLength: 2 },
  { prefix: '@视频', type: 'video', prefixLength: 3 },
  { prefix: '@音频', type: 'audio', prefixLength: 3 },
];
...
export function findReferenceTokens(text: string, maxImageCount?: number): ReferenceTokenMatch[] {
  ...
  for (let index = 0; index < text.length; index += 1) {
    let matchedPattern: PatternMatch | null = null;

    for (const pattern of REFERENCE_PATTERNS) {
      const prefixSlice = text.slice(index, index + pattern.prefixLength);
      if (prefixSlice === pattern.prefix) {
    ...
    const digitsStart = index + matchedPattern.prefixLength;
