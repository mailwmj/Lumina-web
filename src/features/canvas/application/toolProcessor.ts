...
export class CanvasToolProcessor implements ToolProcessor {
  ...
  async process(
    ...
    if (toolType === NODE_TOOL_TYPES.splitStoryboard) {
      ...
      // Get projectId from options or fall back to current project
      const projectId = typeof options.projectId === 'string'
        ? options.projectId
        : useProjectStore.getState().getCurrentProject()?.id;
  ...
  private async splitStoryboard(
    ...
    // splitGateway.split with projectId persists to project dir, no need to re-persist
    const persistedFrameImages = outputs;
